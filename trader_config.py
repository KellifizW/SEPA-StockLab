...
# ─────────────────────────────────────────────────────────────────────────────
# Currency display / conversion defaults (used by dashboard + IBKR helpers)
ACCOUNT_BASE_CURRENCY = "USD"       # Base currency used for account NAV in app logic
...
# ─────────────────────────────────────────────────────────────────────────────
# POSITION CONTROL — 3-pool allocation + automated exit engine
# Reference: modules/position_controller.py, docs/positioncontrol.md
# ─────────────────────────────────────────────────────────────────────────────

# ── Pool allocation (must sum to 100) ─────────────────────────────────────
PC_POOL_ML_PCT             = 40      # ML 倉佔帳戶%上限
PC_POOL_QM_PCT             = 40      # QM 倉佔帳戶%上限
PC_POOL_FREE_PCT           = 20      # 自由倉佔帳戶%上限 (非 ML/QM 來源的持倉)

# ── Per-pool constraints ──────────────────────────────────────────────────
PC_ML_MAX_POSITIONS        = 4       # ML 倉最多持倉數
PC_QM_MAX_POSITIONS        = 4       # QM 倉最多持倉數
PC_FREE_MAX_POSITIONS      = 2       # 自由倉最多持倉數
PC_TOTAL_MAX_POSITIONS     = 8       # 全帳戶硬上限

# ── Portfolio heat (total open risk %) ────────────────────────────────────
PC_ML_MAX_HEAT_PCT         = 2.0     # ML 池最大合計風險% (Martin: ≤3%)
PC_QM_MAX_HEAT_PCT         = 3.0     # QM 池最大合計風險%
PC_FREE_MAX_HEAT_PCT       = 1.5     # 自由倉最大合計風險%
PC_TOTAL_MAX_HEAT_PCT      = 5.0     # 全帳戶最大合計風險% (Kristjan: 3-6%)

# ── Drawdown protection (帳戶級) ──────────────────────────────────────────
PC_LOSS_STREAK_HALVE       = 3       # 連續虧損 >= 3 次 → 所有新倉大小減半
PC_LOSS_STREAK_STOP        = 5       # 連續虧損 >= 5 次 → 全面暫停自動交易
PC_DRAWDOWN_REDUCE_PCT     = 5.0     # 帳戶回撤 >= 5% → 新倉大小減半
PC_DRAWDOWN_STOP_PCT       = 10.0    # 帳戶回撤 >= 10% → 完全停止自動交易

# ── Pool name constants ───────────────────────────────────────────────────
PC_POOL_NAMES              = ["ML", "QM", "FREE"]

# ─────────────────────────────────────────────────────────────────────────────
# EXIT ENGINE — Automated sell / trailing stop execution
# ─────────────────────────────────────────────────────────────────────────────
EXIT_ENGINE_ENABLED            = True       # 是否啟用自動出場引擎
EXIT_CHECK_INTERVAL_SEC        = 60         # 出場檢查間隔秒數 (每分鐘)
EXIT_DRY_RUN                   = True       # True = 只記錄不下單; False = 實際賣出

# ── Time stop (無動作止損) ────────────────────────────────────────────────
EXIT_TIME_STOP_DAYS            = 5          # 買入後 N 天無正向進展 → 出場 (Kristjan: 3-5天)
EXIT_TIME_STOP_MIN_GAIN_PCT    = 1.0        # 至少要漲 N% 才算「有進展」

# ── Climax / Parabolic detection ──────────────────────────────────────────
EXIT_CLIMAX_SINGLE_DAY_PCT     = 15.0       # 單日漲幅 >= N% → 可能高潮頂
EXIT_CLIMAX_VOL_MULT           = 2.5        # 成交量 >= N倍 20日均量 → 配合高潮
EXIT_CLIMAX_EXHAUSTION_GAP     = True       # 跳空+反轉 → 完全出場

# ── Sell order config ─────────────────────────────────────────────────────
EXIT_ORDER_TYPE                = "MKT"      # MKT = 市價賣出 (fastest exit)
EXIT_MAX_SELLS_PER_CYCLE       = 3          # 每輪檢查最多賣出 N 筆 (防雪崩)
