...
AUTO_TRADE_MAX_BUYS_PER_DAY  = 3       # 每日最多自動買入數 (deprecated: use per-strategy caps below)
AUTO_TRADE_MIN_STAR_QM       = 3.5     # [backward-compat alias] → AUTO_TRADE_MIN_SCREENER_STAR_QM
AUTO_TRADE_MIN_STAR_ML       = 3.5     # [backward-compat alias] → AUTO_TRADE_MIN_SCREENER_STAR_ML
...
AUTO_TRADE_LMT_BUFFER_PCT    = 0.1     # 固定限價緩衝% (當 LMT_USE_ATR=False 時使用)
...
# ── Phase gate: screener (Phase 2) vs deep-analysis (Phase 3) 雙重門檻 ────────
# Phase 2 (screener pre-filter): 快速過濾，用掃描結果的星級
AUTO_TRADE_MIN_SCREENER_STAR_QM  = 3.5  # QM Phase 2 screener gate
AUTO_TRADE_MIN_SCREENER_STAR_ML  = 3.5  # ML Phase 2 screener gate
# Phase 3 (deep analysis gate): 執行前必須通過的更嚴格深度分析星級
AUTO_TRADE_MIN_DEEP_STAR_QM      = 4.0  # QM Phase 3 deep-analysis gate (stricter)
AUTO_TRADE_MIN_DEEP_STAR_ML      = 4.0  # ML Phase 3 deep-analysis gate (stricter)

# ── ML CAUTION mode 謹慎模式處理 ────────────────────────────────────────────
AUTO_TRADE_ML_CAUTION_SCORE_BOOST = 15  # CAUTION 時額外需要的 watch score (70+15=85 才執行)
AUTO_TRADE_ML_CAUTION_SIZE_MULT   = 0.5 # CAUTION 時倉位縮減至 50%

# ── Trading hours gate 交易時段控制 (US Eastern Time) ────────────────────────
AUTO_TRADE_START_TIME_ET     = "09:45"  # 最早下單時間 (跳過開盤首 15 分鐘高波動)
AUTO_TRADE_END_TIME_ET       = "15:30"  # 最晚下單時間 (收盤前 30 分鐘停止)

# ── Data freshness 數據時效性 ──────────────────────────────────────────────────
AUTO_TRADE_SCAN_MAX_AGE_MIN  = 30       # 掃描數據最大允許過期時間 (分鐘); 超過則跳過執行

# ── Portfolio-level risk controls 投資組合風險控制 ────────────────────────────
AUTO_TRADE_MAX_TOTAL_EXPOSURE_PCT = 60.0  # 最大總持倉佔帳戶比例 (超過則不新開倉)
AUTO_TRADE_CHECK_BUYING_POWER     = True  # 下單前核查 IBKR 購買力是否足夠

# ── Order lifecycle 訂單生命週期 ──────────────────────────────────────────────
AUTO_TRADE_STOP_ATTACH_CRITICAL = True  # True=若止損單附掛失敗則作廢買入紀錄 (防裸倉)

# ── LMT buffer: ATR-based dynamic adjustment 動態限價緩衝 ────────────────────
AUTO_TRADE_LMT_USE_ATR       = True    # 使用 ATR 動態緩衝 (overrides LMT_BUFFER_PCT)
AUTO_TRADE_LMT_ATR_MULT      = 0.10   # 緩衝 = ATR14 × 此倍數 (default: 10% of ATR)

# ── Per-strategy daily buy quotas 按策略獨立計算每日買入配額 ─────────────────
AUTO_TRADE_MAX_BUYS_PER_DAY_QM = 2     # QM 策略每日最多買入數
AUTO_TRADE_MAX_BUYS_PER_DAY_ML = 2     # ML 策略每日最多買入數

# ── Heartbeat monitoring 心跳監控 ────────────────────────────────────────────
AUTO_TRADE_HEARTBEAT_MAX_SEC = 600     # 超過此秒數無活動則發出 Telegram 告警 (10 分鐘)
